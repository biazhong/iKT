	...
	public void run() {
		...
		while(latestCallTime <= 12) {
			...
			if(truck[selectedTruck][0]>call[i][0]) {
				call[i][3] = truck[selectedTruck][0]-muServe1*Math.log(1-R.nextDouble())
						+distance[selectedTruck]/vt-muServe2*Math.log(1-R.nextDouble());
				
				call[i][4] = call[i][3]+distanceBack/vt;
				truck[selectedTruck][0]=call[i][4];
			}else {
				call[i][3] = call[i][0]-muServe1*Math.log(1-R.nextDouble())
						+distance[selectedTruck]/vt-muServe2*Math.log(1-R.nextDouble());
				call[i][4] = call[i][3]+distanceBack/vt;
				truck[selectedTruck][0]=call[i][4];
			}
