...
public class tAmbulance {
	...
	public tAmbulance(int sp, int ep,double muCall, double muServe,
	...
	//Return best alternative
		...
		public void run() {
			...
			double[][] S1 = new double[I.size()][I.size()];
			...
			for(int i = 0 ; i <I.size(); i++) {
				for(int j = i + 1; j < I.size(); j++) {
					for(int count = 0 ; count < n0Prime; count++) {
						S1[i][j]=S1[i][j]+tempX[i][count]-tempX[j][count];
						S2[i][j]=S2[i][j]+(tempX[i][count]-tempX[j][count])*(tempX[i][count]-tempX[j][count]);
					}
					S1[j][i]=S1[i][j];
					S2[j][i]=S2[i][j];
				}
			...
			int r = 2;
			while(I.size()>1) {
				TDistribution TD = new TDistribution(t-1.0);
				...
				for(int i = 0 ; i < I.size(); i++) {
					...
					//}
					for(int j = i + 1 ; j < I.size(); j++) {
						double Sij = (S2[I.get(i)][I.get(j)]-S1[I.get(i)][I.get(j)]*S1[I.get(i)][I.get(j)]/t)/(t-1.0);
						
						double boundary= Math.sqrt(t*Sij)*TD.inverseCumulativeProbability(1-alpha/((k-1)*r*r));
						double Zij = X[I.get(i)]-X[I.get(j)];
						//System.out.println(t+" "+Zij+" "+X[I.get(i)]+" "+X[I.get(j)]+" "+S1[i][j]);
						//System.out.println(boundary+" "+Zij+" "+t+" "+I.size());
				...
				double[][] tempX1 = new double[k][(int)Math.pow(2, r)];
				for(int count = 0; count < Math.pow(2, r);count++) {
					for(int i = 0 ; i < I.size(); i++) {
						if(I.get(i)==0) {
							if(refSamples.size()>t) {
								X[I.get(i)] = X[I.get(i)] + refSamples.get(t);
								tempX1[I.get(i)][count]=refSamples.get(t);
							...
							tempX1[I.get(i)][count]=genObv.getPerOfGC();
						}	
					}
					t++;
				}
				...
				for(int i = 0 ; i <k; i++) {
					for(int j = i + 1; j < k; j++) {
						for(int count = 0 ; count < Math.pow(2, r); count++) {
							S1[i][j]=S1[i][j]+tempX1[i][count]-tempX1[j][count];
							S2[i][j]=S2[i][j]+(tempX1[i][count]-tempX1[j][count])*(tempX1[i][count]-tempX1[j][count]);
						}
						S1[j][i]=S1[i][j];
						S2[j][i]=S2[i][j];
					}
