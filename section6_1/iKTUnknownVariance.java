...
public class iKTUnknownVariance {
	...
	public void run() {
		...
		while(I.size()>1) {
			...
			if(refAlt == 5 && roundToSelectRef <0) {
				roundToSelectRef =  round;
			}
			...
			for(int i = 0;  i < numOfMatches; i++) {
				if(i < numOfMatches - 1) {
					...
					if(round ==1) {
						if(groupMu.get(0)>=groupMu.get(1)&&groupMu.get(0)>=groupMu.get(2)) {
							PB = PB+1;
						}
					}
					if(round ==1) {
						if(groupMu.get(0)>=groupMu.get(1)+delta&&groupMu.get(0)>=groupMu.get(2)+delta) {
							PA = PA+1;
						}
					}
					...
					if(round ==1) {
						if(groupMu.get(0)>=groupMu.get(1)) {
							PB = PB+1;
						}
					}
					if(round ==1) {
						if(groupMu.get(0)>=groupMu.get(1)+delta) {
							PA = PA+1;
						}
					}
		...
		PB = PB/(Math.ceil(k*1.0/2.0));
		PA = PA/(Math.ceil(k*1.0/2.0));
		bestID=I.get(0);
	}
