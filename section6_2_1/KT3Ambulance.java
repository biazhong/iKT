...
public class KT3Ambulance {
	...
	public KT3Ambulance(int sp, int ep,double muCall, double muServe,
	...
	public void run() {
		...
		while(I.size()>1) {
			...
			int numOfMatches=(int)Math.floor(I.size()*1.0/3.0); 
			totalNumberOfMatches = totalNumberOfMatches+numOfMatches;
			if((I.size()-1)*1.0/3.0==(int)Math.floor(I.size()*1.0/3.0)) {
				tempI.add(I.get(I.size()-1));
				I.remove(I.size()-1);
			}
			if((I.size()-2)*1.0/3.0>(int)Math.floor(I.size()*1.0/3.0)) {
