...
public class RngStream { 
	...
	/**
	 * Generates a Poisson random number for large lambda
	 *
	 ...
	 */
	private int RandPoisson_large_lambda (double lambda) {
		...
		while(true)
		{
			...
			double rhs = k + n*Math.log(lambda) - Gamma.logGamma( n+1 );
			if (lhs <= rhs)
				return n;
		}
	}
	...
	/**
	 * Generates a Poisson random number for small lambda
	 *
	 *
	 ...
	 * @return A Poisson random number with parameter lambda
	 */
	private int RandPoisson_small_lambda (double lambda) {
		double p = 1.0;
	...
	/**
	 * Generates a Poisson random number by selecting the appropriate method based on lambda
	 *
	 * @param lambda
	 * @return A Poisson random number with parameter lambda
	 */
	public int RandPoisson (double lambda) {
		return (lambda < 30) ?
