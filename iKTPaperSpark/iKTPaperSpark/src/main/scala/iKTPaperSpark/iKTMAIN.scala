...
object iKTMAIN {
  def withinCoreSelection( muCall:Double,muServe1:Double,muServe2:Double,alpha:Double,delta:Double
      ,CoreID:Int,nCores:Int,seed:Int,n0Prime:Int,n0:Int,acc_sim_t: Accumulator[Double],
      acc_sample_size: Accumulator[Double]):(Int,Int)={
    val iKT = new iKTTruck(muCall,muServe1,muServe2,alpha,delta,CoreID,nCores,seed+CoreID,n0Prime,n0);
    iKT.run();
    acc_sim_t += iKT.getSimulationTime();
    acc_sample_size +=iKT.getSampleSize;
    
    (iKT.getBestID(),iKT.getMaxRound);
  }
  ...
  def main(args:Array[String]):Unit={
    ...
    while (count < repeat){
      ...
      val coreOutPut = coreID.map(withinCoreSelection(muCall,muServe1,muServe2,alpha,delta,_,nCores,R.nextInt(),
          n0Prime,n0,accum_sim_t,accum_sample_size)).cache();
